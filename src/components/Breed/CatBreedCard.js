...
function CatBreedCard({ breed }) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col h-full">
      {/* Imagem */}
      {imageUrl && (
      ...
      <div className="p-6 flex-1 flex flex-col">
        ...
        <div className="grid grid-cols-2 gap-3 mt-auto pt-4 border-t border-gray-100">
          <div className="text-center">
            <div className="text-xs text-gray-500 mb-1">Afeto</div>
            <div className="flex justify-center gap-1">
              {[...Array(5)].map((_, i) => (
                <div
                  key={i}
                  className={`w-2 h-2 rounded-full ${
                    i < breed.affection_level ? 'bg-pink-500' : 'bg-gray-300'
                  }`}
                />
              ))}
            </div>
            ...
            <div className="flex justify-center gap-1">
              {[...Array(5)].map((_, i) => (
                <div
                  key={i}
                  className={`w-2 h-2 rounded-full ${
                    i < breed.energy_level ? 'bg-orange-500' : 'bg-gray-300'
                  }`}
                />
              ))}
            </div>
          ...
          {breed.intelligence && (
            <div className="text-center">
              <div className="text-xs text-gray-500 mb-1">Inteligência</div>
              <div className="flex justify-center gap-1">
                {[...Array(5)].map((_, i) => (
                  <div
                    key={i}
                    className={`w-2 h-2 rounded-full ${
                      i < breed.intelligence ? 'bg-blue-500' : 'bg-gray-300'
                    }`}
                  />
                ))}
              </div>
          ...
          {breed.child_friendly && (
            <div className="text-center">
              <div className="text-xs text-gray-500 mb-1">C/ Crianças</div>
              <div className="flex justify-center gap-1">
                {[...Array(5)].map((_, i) => (
                  <div
                    key={i}
                    className={`w-2 h-2 rounded-full ${
                      i < breed.child_friendly ? 'bg-green-500' : 'bg-gray-300'
                    }`}
                  />
                ))}
              </div>
